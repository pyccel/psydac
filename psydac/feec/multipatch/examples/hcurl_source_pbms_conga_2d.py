...
from psydac.feec.pull_push import pull_2d_hcurl, pull_2d_hdiv
...
def solve_hdiv_source_pbm(
    ...
    print('---------------------------------------------------------------------------------------------------------')
    print('Starting solve_hdiv_source_pbm function with: ')
    print(' ncells = {}'.format(ncells))
    print(' degree = {}'.format(degree))
    print(' domain_name = {}'.format(domain_name))
    print(' source_proj = {}'.format(source_proj))
    ...
    def lift_u_bc(u_bc):
        if u_bc is not None:
            ...
            u_bc_x = lambdify(domain.coordinates, u_bc[0])
            u_bc_y = lambdify(domain.coordinates, u_bc[1])
            u_bc_log = [pull_2d_hdiv([u_bc_x, u_bc_y], m) for m in mappings_list]
            # it's a bit weird to apply P1 on the list of (pulled back) logical fields -- why not just apply it on u_bc ?
            uh_bc = P1(u_bc_log)
            ubc_c = uh_bc.coeffs.toarray()
            # removing internal dofs (otherwise ubc_c may already be a very good approximation of uh_c ...)
    ...
    print('computing the curl-curl stiffness matrix...')
    print(bD1_m.shape, dH2_m.shape )
    pre_GD_m = bD1_m.transpose() @ dH2_m @ bD1_m
    # GD_m = cP1_m.transpose() @ pre_GD_m @ cP1_m  # Conga stiffness matrix

    # curl curl:
    t_stamp = time_count(t_stamp)
    print('computing the grad-div stiffness matrix...')
    pre_CC_m = - dH1_m @ bD0_m @ cP0_m @ H0_m @ cP0_m.transpose() @ bD0_m.transpose() @ dH1_m
    # CC_m = cP1_m.transpose() @ pre_CC_m @ cP1_m  # Conga stiffness matrix
    ...
    if plot_source:
        plot_field(numpy_coeffs=f_c, Vh=V1h, space_kind='hdiv', domain=domain, title='f_h with P = '+source_proj, filename=plot_dir+'/fh_'+source_proj+'.png', hide_plot=hide_plots)
    ...
    if plot_dir:
        plot_field(numpy_coeffs=uh_c, Vh=V1h, space_kind='hdiv', domain=domain, title=title, filename=plot_dir+params_str+'_uh.png', hide_plot=hide_plots)
...
if __name__ == '__main__':
    ...
    source_type = 'test_Laplacian_sin'
    # source_type = 'manu_J'
