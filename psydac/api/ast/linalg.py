...
class LinearOperatorDot(SplBasic):
    ...
    def __hashable_new__(cls, ndim, block_shape, comm_id=None, **kwargs):
        ...
        obj._generate_code(backend=backend, parallel=parallel)
        obj._compile(backend=backend, parallel=parallel)
    ...
    def _generate_code(self, backend=None, parallel=True):

        modname = 'dependencies_{}'.format(self.tag)

        if self.comm is None or self.comm.rank == 0 or parallel==False:
    ...
    def _compile(self, backend=None, parallel=True):
        ...
        if backend and backend['name'] == 'pyccel':
            package = self._compile_pyccel(package, backend, parallel=parallel)

        self._func = getattr(package, self.name)

    def _compile_pyccel(self, mod, backend, parallel=True, verbose=False):
