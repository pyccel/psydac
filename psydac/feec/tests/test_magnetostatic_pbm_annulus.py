...
def test_constant_one():
    ...
    B_h_eval = V1h.eval_fields(eval_grid, B_h)
    print(B_h_eval)
    assert abs( B_h_eval[0][0][0,1] - (0.25+1) * (np.sin(np.pi)+np.cos(np.pi))) < 0.01, f"B_h_eval[0][0][0,1] - (0.25+1) * (np.sin(np.pi)+np.cos(np.pi)):{B_h_eval[0][0][0,1] - (0.25+1) * (np.sin(np.pi)+np.cos(np.pi))}"
    assert abs( B_h_eval[0][1][0,1] - (np.cos(np.pi) -  np.sin(np.pi))) < 0.01
    assert abs( B_h_eval[0][0][2,1] - (0.75+1) * (np.sin(np.pi)+np.cos(np.pi))) < 0.01
