...
def get_source_and_solution_OBSOLETE(source_type=None, eta=0, mu=0, nu=0,
    ...
    if source_type == 'manu_J':
        ...
        # use a manufactured solution, with ad-hoc (homogeneous or
        # inhomogeneous) bc
        ...
        # boundary condition: (here we only need to coincide with u_ex on the
        # boundary !)
        ...
        if not domain_name in ['pretzel', 'pretzel_f']:
            print(
                "WARNING (87656547) -- I'm not sure we have an exact solution -- check the bc's on the domain " +
                domain_name)
        ...
        # see Beirão da Veiga, Brezzi, Dassi, Marini and Russo, Virtual Element
        # approx of 2D magnetostatic pbms, CMAME 327 (2017)
        ...
        if domain_name in ['pretzel', 'pretzel_f']:
            ...
            # dsigphi2 = 0.005   # if needed: smaller support for phi, to have
            # a smaller value at boundary
