...
class DiagGrid():
    ...
    def __init__(self, mappings=None, N_diag=None):
        ...
        etas, xx, yy, patch_logvols = get_plotting_grid(mappings, N=N_diag, centered_nodes=True, return_patch_logvols=True)
        quad_weights = get_grid_quad_weights(etas, patch_logvols, mappings_list)
    ...
    def grid_vals_hcurl(self, v):
        return get_grid_vals(v, self.etas, self.mappings_list, space_kind='hcurl')
    ...
    def import_ref_sol_from_coeffs(self, sol_ref_filename=None, space='V*'):
        print('[DiagGrid] loading coeffs of ref_sol from {}...'.format(sol_ref_filename))        
        if space == 'V0':
            Vh = self.V0h
        elif space == 'V1':
            Vh = self.V1h
        ...
        self.sol_ref[space] = FemField(Vh, coeffs=array_to_stencil(coeffs, Vh.vector_space))
    ...
    def write_sol_values(self, v, space='V*'):
        ...
        if space in self.sol_vals:
            print('WARNING !! sol_vals[{}] exists -- will be overwritten !! '.format(space))
            print('use refined labels if several solutions are needed in the same space')
        self.sol_vals[space] = get_grid_vals(v, self.etas, self.mappings_list, space_kind=get_kind(space))
    ...
    def compute_l2_error(self, space='V*'):
        if space in ['V0', 'V2']:
            ...
            errors = [np.sqrt( (u1-v1)**2 + (u2-v2)**2 ) for u1, v1, u2, v2 in zip(u_x, uh_x, u_y, uh_y)]
        else:
            raise ValueError(space)                
        
        l2_norm_uh = (np.sum([J_F * v**2 for v, J_F in zip(abs_uh, self.quad_weights)]))**0.5
        l2_norm_u  = (np.sum([J_F * v**2 for v, J_F in zip(abs_u,  self.quad_weights)]))**0.5
        l2_error   = (np.sum([J_F * v**2 for v, J_F in zip(errors, self.quad_weights)]))**0.5
