...
def test_jacobians(geometry_file, ldim, is_nurbs):
    ...
    quad_order, \
    global_basis, \
    global_spans = mapping.space.preprocess(der=1, refine_factor=1)

    jac_mats = np.zeros((*(tuple(ncells[i] * (quad_order[i] + 1) for i in range(ldim))), ldim, ldim))
    inv_jac_mats = np.zeros((*(tuple(ncells[i] * (quad_order[i] + 1) for i in range(ldim))), ldim, ldim))
