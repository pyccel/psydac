...
def run_convergence_tests(domain_name, degree):
    ...
    elif domain_name == 'refined_square':
        domain = [[0, np.pi],[0, np.pi]]
        ncells = [2*np.ones((2,2), dtype=int),
                    2*np.ones((4,4), dtype=int),
                    2*np.ones((6,6), dtype=int),
                    2*np.ones((8,8), dtype=int)]
    ...
    case_dir = 'conv_runs_'+domain_name
    ...
    fig, ax = plt.subplots( )
    ax.loglog()
    ax.set_title("error of eigenvalues over dofs")
