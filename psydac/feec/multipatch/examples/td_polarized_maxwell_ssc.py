...
from sympy import lambdify, Matrix, Tuple
...
from psydac.feec.multipatch.examples.ppc_test_cases     import get_div_free_pulse, get_polarized_solution, get_polarized_source
...
def solve_td_polarized_maxwell_pbm(
        method='ssc',
        nbc=4, deg=4, 
        mom_pres=False, 
        C1_proj_opt=None,
   ...
   # x,y = domain.coordinates
    ...
    v1 = dot(Tuple(1,0),v)
    v2 = dot(Tuple(0,1),v)
    mass = BilinearForm((v,u), integral(domain, ((u1*v1 + u2*v2) + kappa * u1 * v1 )))
    ...
    def project_exact_polarized_solution(t, proj_type='P_geom'):
        ...
        E_ex, B_ex, D_ex, J_ex = get_polarized_solution(omega, kx, ky, kappa, t=t, domain=domain)
    ...
    for nt in range(Nt):
        ...
        Jex_h = P_phys_hdiv(get_polarized_source(omega, kx, ky, kappa, (nt+1/2)*dt, domain), d_geomP1, domain, mappings_list)
        J_c[:] = Jex_h.coeffs.toarray()
