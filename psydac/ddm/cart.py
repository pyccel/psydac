...
class InterfaceCartDecomposition:
    ...
    def __init__(self, cart_minus, cart_plus, comm, axes, exts, ranks_in_topo, local_groups, local_communicators, root_ranks, requests, reduce_elements=False):
        ...
        root_minus = comm.group.Translate_ranks([0], boundary_group_minus)[0]
        root_plus  = comm.group.Translate_ranks([0], boundary_group_plus)[0]

        procs_index_minus = boundary_group_minus.Translate_ranks(self._boundary_ranks_minus, local_groups[0])
        procs_index_plus  = boundary_group_plus.Translate_ranks(self._boundary_ranks_plus, local_groups[1])
        ...
        self._local_boundary_ranks_minus = boundary_group_minus.Translate_ranks(self._boundary_ranks_minus, local_groups[0])
        self._local_boundary_ranks_plus  = boundary_group_plus.Translate_ranks(self._boundary_ranks_plus, local_groups[1])
...
def create_interfaces_cart(domain_decomposition, carts, interfaces, communication_info):
    ...
    for i,j in interfaces:
        ...
        if i in owned_groups or j in owned_groups:
            ...
            root_rank_i            = interfaces_groups[i,j].Translate_ranks([0], local_groups[i])[0]
            root_rank_j            = interfaces_groups[i,j].Translate_ranks([0], local_groups[j])[0]
