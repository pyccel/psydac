...
def transpose_1d(M  : "T[:,:]",
    ...
    # -- removed omp: #$ omp parallel default(private) shared(Mt,M) firstprivate( n,nc,gp,p,dm,cm,nd,ndT,si,sk,sl)
    d1 = gp[0] - p[0]
    e1 = nd[0] - sl[0]
    # -- removed omp: #$ omp for schedule(static) collapse(1)
    for x1 in range(n[0]):
...
def transpose_2d(M  : "T[:,:,:,:]",
    ...
    # -- removed omp: #$ omp for schedule(static) collapse(2)
    for x1 in range(n[0]):
        for x2 in range(n[1]):
...
def transpose_3d(M  : "T[:,:,:,:,:,:]",
    ...
    # -- removed omp: #$ omp for schedule(static) collapse(3)
    for x1 in range(n[0]):
        for x2 in range(n[1]):
            for x3 in range(n[2]):
