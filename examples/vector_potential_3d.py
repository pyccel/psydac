...
def compute_vector_potential_3d(b, derham_h):
    ...
    Additionally highlights the usage of `get_LST_preconditioner` and `get_DirichletBoundaryProjector` introduced in this PR.
    ...
    and applying the projection method using `get_DirichletBoundaryOperator` to solve this discrete problem.
    """
    ...
    S3  = C.T @ M2 @ C  +  M1 @ G @ M0_inv   @ DBP0 @ G.T @ M1
    ...
    diff = b - C @ a3
    l2norm_diff = np.sqrt(M2.dot_inner(diff, diff))
    print(f' ----- Option3: C.T @ M2 @ C  +  M1 @ G @ M0_inv   @ DBP0 @ G.T @ M1 -----')
    print()
    print(f' || B - curl(A) || = {l2norm_diff:.3g}')
