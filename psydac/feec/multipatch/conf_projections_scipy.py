...
def glob_ind_interface_scalfield(i, j, side, k_patch, axis, patch_shape, n_patches, nb_dofs_perp):
    ...
    # nb_dofs_perp: nb dofs in direction perpendicular to interface
    if side == 1:
        absolute_j = j 
    else:
        absolute_j = nb_dofs_perp-1-j
...
def glob_ind_interface_vecfield(i, j, side, k_patch, axis, patch_shape, n_patches, nb_dofs_perp, comp):
    # returns global index of dof close to interface, for a scalar field
    #
    # i: dof index along interface
    # j: relative dof index (distance) from interface
...
def smooth_1d_V0_Cr(V0h, conf_axis, reg=0, p_moments=-1, nquads=None, hom_bc=False):
    ...
    reg : order of imposed continuity (0 or 1) 
...
# ===========================================================================================================
#
# temp version for a common C0 / C1 conf proj operator for V0h

def Conf_proj_0_c01(V0h, reg=0, p_moments=[-1,-1], nquads=None, hom_bc=False):
    """
    Create the matrix enforcing the C0 or C1 continuity with moment preservation for the V0 space
    ...
    if n_patches ==1:
        ...
        S_x=smooth_1d_V0_Cr(V0h, conf_axis=0, reg=reg, p_moments=p_moments[0], nquads=nquads, hom_bc=hom_bc)
        S_y=smooth_1d_V0_Cr(V0h, conf_axis=1, reg=reg, p_moments=p_moments[1], nquads=nquads, hom_bc=hom_bc)
        Proj=S_x@S_y
        Proj_op = Operator_from_scipy(V0h.vector_space,V0h.vector_space,Proj)
    return Proj_op.tosparse()
...
def Conf_proj_1_c01(V1h, reg, nquads, hom_bc):
    """
