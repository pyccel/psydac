...
class EvalField(BaseNode):
    ...
    mapping : <BaseAnalyticMapping>
        Sympde BaseAnalyticMapping object
...
class CoefficientBasis(ScalarNode):
    ...
    """
    def __new__(cls, target):
        ls = target.atoms(ScalarFunction, VectorFunction, BaseAnalyticMapping)
        if not len(ls) == 1:
            raise TypeError('Expecting a scalar/vector test function or a BaseAnalyticMapping')
        return Basic.__new__(cls, target)
...
class GeometryAtom(AtomicNode):
    ...
    """
    def __new__(cls, expr):
        ls = list(expr.atoms(BaseAnalyticMapping))
        if not(len(ls) == 1):
