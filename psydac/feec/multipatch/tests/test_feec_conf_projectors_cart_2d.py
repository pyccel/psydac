...
@pytest.mark.parametrize("nonconforming, full_mom_pres", [(True, False), (False, True)])
...
def test_conf_projectors_2d(
    ...
    if full_mom_pres and (nc >= 3 + 2*reg[0]) and (nc >= 3 + 2*reg[1]):
        mom_pres = degree 
    else:
        mom_pres = [-1,-1]
    ...
    assert np.allclose(sp_norm( D0 - cP1@D0),  0, 1e-12, 1e-12) # D0 maps in the conforming V1 space (where cP1 coincides with Id)
    assert np.allclose(sp_norm( D1 - cP2@D1),  0, 1e-12, 1e-12) # D1 maps in the conforming V2 space (where cP2 coincides with Id)
    ...
    assert np.allclose(g0_c,     g0_L2_c, 1e-12, 1e-12) # (P0_geom - P0_L2) polynomial = 0
    assert np.allclose(g0_c, cP0@g0_L2_c, 1e-12, 1e-12) # (P0_geom - confP0 @ P0_L2) polynomial= 0
    ...
    if full_mom_pres:
        ...
        assert np.allclose(g0_c, g0_star_c, 1e-12, 1e-12)  #  (P10_geom - P0_star) polynomial = 0 
    ...
    assert np.allclose(G1_c, cP1 @ G1_L2_c, 1e-12, 1e-12)  # (P1_geom - confP1 @ P1_L2) polynomial= 0
    ...
    if full_mom_pres:
        ...
        assert np.allclose(G1_c, G1_star_c, 1e-12, 1e-12) # (P1_geom - P1_star) polynomial = 0 
    ...
    assert np.allclose(g2_c,       g2_L2_c, 1e-12, 1e-12) # (P2_geom - P2_L2) polynomial = 0
    assert np.allclose(g2_c, cP2 @ g2_L2_c, 1e-12, 1e-12) # (P2_geom - confP2 @ P2_L2) polynomial = 0
    ...
    if full_mom_pres:                
        ...
        assert np.allclose(g2_c, g2_star_c, 1e-12, 1e-12) # (P2_geom - P2_star) polynomial = 0