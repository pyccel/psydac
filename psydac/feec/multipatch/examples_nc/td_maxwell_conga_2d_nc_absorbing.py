...
from psydac.feec.multipatch.examples.ppc_test_cases     import get_source_and_solution_hcurl, get_div_free_pulse, get_curl_free_pulse, get_Delta_phi_pulse, get_praxial_Gaussian_beam_E, get_easy_Gaussian_beam_E, get_easy_Gaussian_beam_B,get_easy_Gaussian_beam_E_2, get_easy_Gaussian_beam_B_2
...
from psydac.feec.multipatch.multipatch_non_conf_scipy import construct_V0_conforming_projection, construct_V1_conforming_projection
...
#        diag_dtau        = None,
    ...
    if domain_name == 'refined_square' or domain_name =='square_L_shape':
        ...
        ncells_h = {patch.name: [nc[int(patch.name[2])][int(patch.name[4])], nc[int(patch.name[2])][int(patch.name[4])]] for patch in domain.interior}
    else:
    ...
    #cP0 = derham_h.conforming_projection(space='V0', hom_bc=False, backend_language=backend, load_dir=m_load_dir)
    #cP1 = derham_h.conforming_projection(space='V1', hom_bc=False, backend_language=backend, load_dir=m_load_dir)
    ...
    if E0_type == 'th_sol':
        ...
        E0 = get_easy_Gaussian_beam_E(x_0=3.14, y_0=0.05, domain=domain)
        ...
        if E0_proj == 'P_geom':
            ...
            tilde_B0_c = derham_h.get_dual_dofs(space='V2', f=B0, backend_language=backend, return_format='numpy_array')
    ...
    stencil_coeffs_E = array_to_stencil(cP1_m @ E_c, V1h.vector_space)
    Eh = FemField(V1h, coeffs=stencil_coeffs_E)
    OM1.add_snapshot(t=0 , ts=0) 
    OM1.export_fields(Eh=Eh)

    stencil_coeffs_B = array_to_stencil(B_c, V2h.vector_space)
