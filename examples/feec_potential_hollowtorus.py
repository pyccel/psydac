...
# ----- Parameters & Settings -----
# Domain Parameters
eps      = 0.15 # inner radius of the toroidal cavity (why "inner" ?)
...
file_name = f'visu_{mapping_name}_eps{eps}_n{ncells[0]}_d{degree[0]}_tol{cor_tol}'
...
if compute:
    ...
    if use_struphy_mapping:
        ...
        if mapping_name == 'hollow_desc':
            str_domain = str_domains.DESCunit(str_equils.DESCequilibrium(use_nfp=False))
        elif mapping_name == 'hollow_gvec':
            str_domain = str_domains.GVECunit(str_equils.GVECequilibrium(rmin=0.5, use_nfp=False))
        elif mapping_name == 'str_hollow_torus':
            str_domain = str_domains.HollowTorus(a1=eps, a2=minRad, R0=majRad) # HollowTorus domain from Struphy
        else:
...
if pv_show:
    ...
    for field_key, field_label in vector_fields:
        ...
        clipped = mesh.clip(normal=(0, 1, 1), origin=(0, 0, 0), invert=False)
