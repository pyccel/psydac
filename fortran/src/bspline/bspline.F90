...
!> Module for Splines 
!> @details
!> mostly wrappers for spl_m_bspline 
!> the user should use this module and not spl_m_bspline
module spl_m_bsp
contains
  ...
  subroutine FindNonZeroElements(p,m,U,n_elements,grid)
    use spl_m_bspline, Find => FindNonZeroElements_bspline
    implicit none
  ...
  subroutine FindMult(p,m,U,uu,span,mult)
    use spl_m_bspline, FindM => FindMult
    implicit none
    integer(kind=4), intent(in)  :: p, m
    real   (kind=8), intent(in)  :: U(0:m), uu
  ...
  subroutine FindSpanMult(p,m,U,uu,k,s)
    use spl_m_bspline, FindSM => FindSpanMult
    implicit none
    integer(kind=4), intent(in)  :: p, m
    real   (kind=8), intent(in)  :: U(0:m), uu
    integer(kind=4), intent(out) :: k, s
  ...
  subroutine InsertKnot(d,n,p,U,Pw,uu,r,V,Qw)
    use spl_m_bspline, InsKnt => InsertKnot
    implicit none
    integer(kind=4), intent(in)  :: d
    integer(kind=4), intent(in)  :: n, p
    real   (kind=8), intent(in)  :: U(0:n+p+1)
  ...
  subroutine RemoveKnot(d,n,p,U,Pw,uu,r,t,V,Qw,TOL)
    use spl_m_bspline, RemKnt => RemoveKnot
    implicit none
    integer(kind=4), intent(in)  :: d
    integer(kind=4), intent(in)  :: n, p
    real   (kind=8), intent(in)  :: U(0:n+p+1)
  ...
  !> @param[in] Qw    Final Control points  
  subroutine RefineKnotVector(d,n,p,U,Pw,r,X,Ubar,Qw)
    use spl_m_bspline, RefKnt => RefineKnotVector
    implicit none
    integer(kind=4), intent(in)  :: d
    integer(kind=4), intent(in)  :: n, p
    real   (kind=8), intent(in)  :: U(0:n+p+1)
  ...
  !> @param[in] Qw Final Control points  
  subroutine DegreeElevate(d,n,p,U,Pw,t,nh,Uh,Qw)
    use spl_m_bspline, DegElev => DegreeElevate
    implicit none
    integer(kind=4), intent(in)  :: d
    integer(kind=4), intent(in)  :: n, p
    real   (kind=8), intent(in)  :: U(0:n+p+1)
  ...
  subroutine Extract(d,n,p,U,Pw,x,Cw)
    use spl_m_bspline, CornerCut => CurvePntByCornerCut
    implicit none
    integer(kind=4), intent(in)  :: d
    integer(kind=4), intent(in)  :: n, p
    real   (kind=8), intent(in)  :: U(0:n+p+1)
